...
The release wheel bundles a prebuilt `mgba_libretro.so` — `pip install retrokix`
...
- Working on `retrokix.libretro` / `retrokix.runtime` without `pip install`-ing a
  wheel.
...
5. Copies the `.so` to `src/retrokix/cores/mgba_libretro.so` and the upstream
   `LICENSE` to `src/retrokix/cores/LICENSE.mGBA`.
6. Stamps `MGBA_VERSION` into `src/retrokix/cores/__init__.py`.
...
5. Commit `.mgba-version`, `src/retrokix/cores/__init__.py` (auto-stamped),
...
- **Don't use mGBA's upstream Python bindings** (`-DBUILD_PYTHON=ON`) —
  ...
  optional features are off, and they break under LTO. retrokix wraps
