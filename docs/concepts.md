...
who wants to know what gbax is *for* before learning what it does.

## The cooperative loop

`gbax play <rom>` opens an SDL window with sound and a keyboard. Pass
...
You launch `gbax play emerald --listen` and a coding agent in the
...
You launch `gbax serve` (no human window) and your training loop. The
...
samus position lives somewhere else. gbax doesn't ship a per-game
memory map. Instead: while you play, press `Ctrl+F` (or POST to
`/capture_state` from an agent), label what's true in this moment
(`hp=22, level=7, scene=overworld`), and gbax records 30 frames of
memory. Repeat 5-10 times at varied states. Run `gbax state compile
<rom>`. gbax intersects your labels against memory and reports the
...
A plugin is a single Python file with a `gbax.plugin()` instance.
...
The bundled `gbax.plugins.emerald_party` plugin decodes the encrypted
...
While the action is running, gbax holds the runtime lock — the SDL
...
- A trained model. gbax is the environment, not the policy.
