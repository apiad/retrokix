...
codes natively — gbax just passes the raw code string through
`retro_cheat_set`. The community database (libretro/libretro-database) of
~6700 named GBA cheats is vendored at `gbax/data/libretro_cheats_gba.json`.
"""
...
def _load_bundled_cheats() -> dict[str, list[Cheat]]:
    blob = _resource_files("gbax.data").joinpath("libretro_cheats_gba.json").read_text()
