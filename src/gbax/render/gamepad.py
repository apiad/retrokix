"""SDL2 gamepad support for the `gbax play` loop.
...
- All connected pads share one input bus (set-union into `held`),
  matching how gbax already merges keyboard + agent HTTP inputs.
...
class PadManager:
    ...
    the HTTP API. That set-union is what gives gbax its 'human + agent
