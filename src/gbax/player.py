...
The protocol is newline-delimited JSON over stdin/stdout:
  ...
  gbax → player   HELLO  {"type":"hello", "scenario":..., "decision_period":..., "schema_version":1}
  player → gbax   READY  {"type":"ready", "name":"...", "persistent":false}

  gbax → player   OBS    {"type":"obs", "frame":N, "data":{...}}
  player → gbax   ACT    {"type":"act", "buttons":["a","right"]}
  (loop)

  gbax → player   DONE   {"type":"done", "result":{...}, "reason":"scored"|"timeout"|"forfeit"|"crashed"}
