...
The downloaded `.zip` per entry is extracted into `~/.gbax/roms/`
...
class ConsoleInfo:
    ...
    rom_exts: tuple[str, ...]
    bundle_json: str   # bundled file under gbax/data/
    cheats_json: str
    core_so: str       # libretro core filename under gbax/cores/
...
def _load_bundle(console_slug: str) -> tuple[str, list[RomEntry]]:
    ...
    blob = _resource_files("gbax.data").joinpath(info.bundle_json).read_text()
...
def resolve_rom(query_or_path: str, roms_dir: Path | None = None) -> Path:
    ...
    if not matches:
        raise FileNotFoundError(
            f"no local ROM matches {query_or_path!r}; try `gbax download {query_or_path!r}` first"
        )
    if len(matches) > 1:
