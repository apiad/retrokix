...
A plugin file constructs a single `Plugin` instance via `gbax.plugin()`,
attaches handlers with decorators, and is loaded by `gbax play
...
def load_plugin(path_or_module) -> Plugin:
    ...
    if len(instances) != 1:
        raise RuntimeError(
            f"plugin file {p.name} must create exactly one gbax.plugin() instance; "
            f"found {len(instances)}"
        )
    return instances[0]
