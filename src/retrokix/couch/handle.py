...
The retrokix play loop is synchronous (one big `while SDL_PollEvent` loop
...
`retrokix couch broker` to run the broker until SIGINT without exposing
...
from retrokix.couch.naming import DEFAULT_ROOM
from retrokix.couch.session import Broker, Client, Event, PeerInfo
...
class BrokerHandle:
    ...
    `retrokix couch broker` and by the auto-spawn helper."""
