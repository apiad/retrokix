"""`retrokix couch ...` CLI — broker, send, listen, whoami, room-code.
...
that lands next uses the same CouchHandle these commands sit on.
  ...
  retrokix couch broker
    Binds ~/.retrokix/couch/default.sock and forwards events between
    connected peers in the same room until SIGINT.

  retrokix couch listen --receives couch.gift.consumable.tool [--room CODE]
    Connects as your persistent identity, prints every event it
    receives in `--room` (or 'default'), one per line.

  retrokix couch send --to PEER_ID --event TYPE --payload JSON [--room CODE]
    Fires one event, disconnects. Sender identity comes from
    ~/.retrokix/identity.json by default.

  retrokix couch whoami
    Print your persistent peer identity.

  retrokix couch room-code
...
from retrokix.couch.identity import load_or_generate as load_identity
...
from retrokix.couch.session import Event

app = typer.Typer(help="Couch — peer-to-peer plugin events.", no_args_is_help=True)


DEFAULT_SOCK = Path.home() / ".retrokix" / "couch" / "default.sock"
...
def listen(
    ...
    peer_id: str = typer.Option(None, "--peer-id", help="Override identity (default: ~/.retrokix/identity.json)."),
