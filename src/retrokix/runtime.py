...
Wraps the libretro shim and exposes a clean, retrokix-shaped API. Single source
of truth for emulator state — both the SDL renderer (for `retrokix play`) and
the FastAPI server (for `retrokix serve`) are clients of this class.
"""
...
from retrokix.libretro import LibretroCore
...
def _default_core_path(rom_path: Path | None = None) -> Path:
    ...
    1. ``$RETROKIX_CORE_PATH`` — explicit override (power users, alt cores).
    2. Bundled ``retrokix/cores/<core>.so`` — what ``pip install`` ships.
    ...
    """
    env = os.environ.get("RETROKIX_CORE_PATH")
    if env:
        return Path(env)
...
class EmulatorRuntime:
    def __init__(
        ...
        self._save_dir = Path(save_dir) if save_dir else Path.home() / ".retrokix" / "saves"
        if not self._core_path.exists():
            raise FileNotFoundError(
                f"libretro core not found at {self._core_path}. "
                "Options: (1) `pip install retrokix` on Linux x86_64 to get the "
                "bundled core, (2) set $RETROKIX_CORE_PATH to an existing .so, "
        ...
        # Per-ROM hotkey pins (F1..F9 → cheat slug), loaded from ~/.retrokix/pins/<sha1>.json
        ...
        self._playing_macro: object | None = None  # retrokix.macros.Macro
    ...
    def _hydrate_slots_from_disk(self) -> None:
        """Load every persisted slot from ~/.retrokix/saves/<rom-sha1>/ into memory."""
    ...
    def stop_recording_macro(self):
        """Return a retrokix.macros.Macro, or None if no recording was active.
    ...
    def console(self) -> str | None:
        ...
        None if the extension isn't one retrokix recognises."""
