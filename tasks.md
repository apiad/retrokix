...
- [ ] **Promote `--tui` to default + port Ctrl+F/Ctrl+R to TUI modals** — the companion TUI works behind opt-in `--tui`; making it the default play surface needs the two terminal-`input()` hotkey flows (Ctrl+F capture labels, Ctrl+R macro name/bind) reimplemented as Textual modal screens so they no longer need the raw terminal. Then drop `interactive=False` gating. ~half day. Spec: [[2026-06-22-retrokix-tui-shell-and-pokedex-design]].
- [ ] **Pokédex caught/seen overlay** (slice 5) — wire the `shared/pokedex.py` stub: decode `dexSeen`/`dexCaught` bitfields from SaveBlock1, read live via the in-process runtime, and render a ✓ Caught line + completion stats in the Pokédex tab (slot already reserved in the detail layout). Depends on the Emerald plugin's live-memory path. ~half day. Spec: [[2026-06-22-retrokix-tui-shell-and-pokedex-design]].
